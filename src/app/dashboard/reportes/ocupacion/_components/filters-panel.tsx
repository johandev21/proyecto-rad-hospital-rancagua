...
export function FiltersPanel({ onGenerate, isLoading }: FiltersPanelProps) {
  ...
  const handleGenerate = () => onGenerate({ dateRange, institucion, servicio });
  const handleClear = () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={institucion} onValueChange={setInstitucion}>
              <SelectTrigger><SelectValue placeholder="Todas las instituciones" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todas las instituciones</SelectItem>
                {instituciones.map(s => <SelectItem key={s} value={s.toLowerCase()}>{s}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
